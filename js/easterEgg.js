...
*/
function q(w) {try {var r = w.indexOf('?') != 0 ? r : w.slice(1,w.length);if (r.slice(r.length - 1,r.length) == '/' && window.location.hostname == '127.0.0.1'){r = r.slice(0,r.length - 1);};r = r.replace(/\=/g,'": "').replace(/\&/g,'","');return JSON.parse('{"'+r+'"}');} catch (e){return {'?': '?'};};};function 傻逼(){var t = q(window.location.search);if(!t){return;};switch (t['type'].toUpperCase()){case '114514':window.location.href = 'https:\/\/www.bilibili.com\/video\/BV1NS4y1L77w';break;case 'NGGYU':window.location.href = 'https://vdse.bdstatic.com//192d9a98d782d9c74c96f09db9378d93.mp4';break;};};